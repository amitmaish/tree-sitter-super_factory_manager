...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_super_factory_manager", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_super_factory_manager": ["*.pyi", "py.typed"],
        "tree_sitter_super_factory_manager.queries": ["*.scm"],
    },
    ext_package="tree_sitter_super_factory_manager",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_super_factory_manager/binding.c",
                "src/parser.c",
            ],
            define_macros=[
